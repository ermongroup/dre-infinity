...
class ResidualBlockv1(nn.Module):
  def __init__(self, input_dim, output_dim, resample=None, act=nn.SiLU(),
               normalization=nn.GroupNorm, adjust_padding=False,
               dilation=None):
    super().__init__()
    self.non_linearity = act
    self.input_dim = input_dim
    ...
    if resample == 'down':
      if dilation is not None:
        ...
        # self.normalize2 = normalization(input_dim)
        self.normalize2 = normalization(num_groups=min(output_dim, 32),
                                    num_channels=input_dim, eps=1e-6)
      ...
      if dilation is not None:
        ...
        # self.normalize2 = normalization(output_dim)
        self.normalize2 = normalization(num_groups=min(output_dim, 32),
                                    num_channels=output_dim, eps=1e-6)
        self.conv2 = dilated_conv3x3(output_dim, output_dim,
                                     dilation=dilation)
      else:
        # conv_shortcut = nn.Conv2d ### Something wierd here.
    ...
    # self.normalize1 = normalization(input_dim)
    self.normalize1 = normalization(num_groups=min(output_dim, 32),
                                    num_channels=input_dim, eps=1e-6)
