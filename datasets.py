  ...
  if config.data.dataset in ['FFHQ', 'CelebAHQ']:
    def preprocess_fn(d):
      ...
      if uniform_dequantization:
        img = (tf.random.uniform(img.shape,
                                 dtype=tf.float32) + img * 255.) / 256.
...
def get_dataset_for_flow(config, uniform_dequantization=False):
  ...
  dataset = MNIST(os.path.join(data_dir, 'datasets', 'mnist'),
                  train=True,
                  download=True,
  ...
  train_ds = torch.utils.data.DataLoader(
    train_ds, config.training.batch_size,
    shuffle=True, num_workers=2, drop_last=False)
  eval_ds = torch.utils.data.DataLoader(
    eval_ds, config.eval.batch_size,
