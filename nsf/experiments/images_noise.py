...
#                      use_resnet, num_res_blocks, resnet_batchnorm, dropout_prob,
#                      ):
def create_transform(c, h, w, train_mean, val_mean, train_cov, val_cov,
                     levels, hidden_channels, steps_per_level, alpha, num_bits, preprocessing,
                     multi_scale,
                     actnorm, coupling_layer_type, spline_params,
                     use_resnet, num_res_blocks, resnet_batchnorm, dropout_prob,
    ...
    linear_transform = transforms.AffineTransformv2(shape=(784,), shift=torch.zeros(784), scale=train_cov)
...
def create_flow(c, h, w,
                train_mean, val_mean, train_cov, val_cov,
                flow_checkpoint, _log):
    distribution = distributions.StandardNormal((c * h * w,))
    # transform = create_transform(c, h, w)
    train_transform, val_transform, transform = create_transform(c, h, w, train_mean, val_mean, train_cov, val_cov)
...
@ex.capture
def train_flow(flow, train_dataset, val_dataset, test_dataset, dataset_dims, device,
               ...
               optimizer_checkpoint, start_step, eta_min, _log, weight_decay):
    """
    ...
    for step, (batch, _) in enumerate(load_num_batches(loader=train_loader,
        ...
        if step > 2000 and step % intervals['early_stop'] == 0 and (test_loader is not None):
            ...
            test_log_prob = autils.eval_log_density(log_prob_fn=log_prob_fn,
                                                   data_loader=test_loader)
            test_log_prob = nats_to_bits_per_dim(test_log_prob).item()

            _log.info("It: {}/{} test_log_prob: {:.3f}".format(step, num_steps, test_log_prob))
            ...
            if eps < best_eps:
                countdown = True
                best_eps = eps
                torch.save(flow.state_dict(), os.path.join(run_dir, 'flow_early_stop.pt'))
                _log.info('It: {}/{} best eps improved, saved flow_early_stop.pt'
                          .format(step, num_steps))
            else:
                if countdown:
                    patience -= 1
                    if patience == 0:
                        _log.info('Early stopping at step {}, best eps is {}'.format(step, best_eps))
...
def eval_on_test(batch_size, num_workers, seed, _log):
    ...
    out_dir = '/atlas/u/kechoi/time-score-dre/nsf/runs/images/noise/'

    # get other things
    train_dataset, val_dataset, (c, h, w) = get_train_valid_data()
    train_mean, val_mean, train_cov, val_cov = get_data_stats(train_dataset, val_dataset)
    ...
    flow, train_transform, test_transform = create_flow(c, h, w, train_mean, test_mean, train_cov, train_cov)
    flow = flow.to(device)
    ...
    samples /= 256.
    save_image(samples.cpu(), os.path.join(out_dir, 'samples.png'),
               nrow=8,
               padding=0)
...
def main(seed, _log):
    ...
    train_transform, val_transform, flow = create_flow(c, h, w, train_mean, val_mean, train_cov, val_cov)
